...
## K trained leaf centroids dropped by hierarchical k-means — OPEN (found 2026-07-03)
...
- **Status:** OPEN — deliberately deferred; fix belongs in Calvin's 3760 via the Gerrit
  amendment flow. Fix sketch: emit the true-leaf level (key -1) while preserving bottom-up
  emission order and BFS-from-root ids, then tighten the unit test to require K leaf tuples.
