...
- [x] `bug-archive.md` — significant bugs with root cause + status; currently: flush leaf-as-root (fixed), quantized_distance field-2 semantics (OPEN), k-means leaf-level drop (OPEN), training-sample run-file leak (fixed), plus a latent-risks/smells section; legacy bugs to migrate
