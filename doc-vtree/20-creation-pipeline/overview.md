...
- Upper levels are built by re-clustering each level's centroids until one level fits a
  ...
  highest.
- ⚠ **Open bug:** for K ≥ 4 the K trained leaf centroids are re-keyed to level −1 and never
  emitted — the tree gets ~√K leaves (see [bug-archive](../60-quality/bug-archive.md)).
