...
public class VectorDistanceScalarEvaluator implements IScalarEvaluator {
    ...
    public VectorDistanceScalarEvaluator(IEvaluatorContext context, final IScalarEvaluatorFactory[] evaluatorFactories,
        ...
        this.decoder = new VectorListDecoder(sourceLoc, funcId);
        this.ctx = context;
        ...
        try {
            ...
            if (evaluatorFactories[0] instanceof ConstantEvalFactory) {
                constantLeft = true;
                ATypeTag leftArgType = resolveConstantArgType(evaluatorLeft, pointableLeft, listAccessorLeft);
                if (leftArgType != null) {
                    vectorArgsLeft = decoder.createArrayFromList(listAccessorLeft,
            ...
            if (evaluatorFactories[1] instanceof ConstantEvalFactory) {
                constantRight = true;
                ATypeTag rightArgType = resolveConstantArgType(evaluatorRight, pointableRight, listAccessorRight);
                if (rightArgType != null) {
                    vectorArgsRight = decoder.createArrayFromList(listAccessorRight,
    ...
    private ATypeTag resolveConstantArgType(IScalarEvaluator evaluator, IPointable pointable, ListAccessor listAccessor)
            throws HyracksDataException {
        ...
        listAccessor.reset(pointable.getByteArray(), pointable.getStartOffset());
        ATypeTag argType = decoder.resolveArgType(listAccessor);
        return (argType != null && decoder.isNumericType(argType)) ? argType : null;
    }
