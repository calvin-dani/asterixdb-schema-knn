...
public final class HierarchicalKMeansPlusPlusCentroidsOperatorDescriptor extends AbstractOperatorDescriptor {
    ...
    private static final class BottomUpClusterStructure {
        ...
        void initializeParentLevel(int level, int parentCount) {
            List<CentroidInfo> parentLevel = new ArrayList<>();
            Map<Integer, List<Integer>> parentChildMap = new HashMap<>();
            for (int i = 0; i < parentCount; i++) {
                parentLevel.add(new CentroidInfo(i, -1, null, level));
        ...
        void buildLevelFromAssignments(List<double[]> childCentroids, List<double[]> parentCentroids, int[] assignments,
            ...
            for (int i = 0; i < assignments.length; i++) {
                int parentClusterId = assignments[i];
                int childId = i;
        ...
        void outputHierarchicalStructure(FrameTupleAppender appender, IFrameWriter writer, IHyracksTaskContext ctx)
        ...
        static long calculateEstimatedTupleSize(int embeddingDimension) {
            return 38L + 13L * embeddingDimension;
        }

        static boolean doesLevelFitInFrame(int centroidCount, int embeddingDimension, int frameSize) {
            ...
            long frameOverhead = 9L + (4L * centroidCount);
    ...
    public HierarchicalKMeansPlusPlusCentroidsOperatorDescriptor(IOperatorDescriptorRegistry spec,
            ...
            int maxScalableKmeansIter, String distanceMetric, int vectorDimension) {
        super(spec, 1, 1);
    ...
    private boolean requiresNormalizedCentroids() {
        return requiresNormalizedCentroids(distanceFunction);
    }

    private void maybeNormalizeCentroid(double[] centroid) {
        maybeNormalizeCentroid(centroid, distanceFunction);
    }
            ...
            return new AbstractUnaryOutputSourceOperatorNodePushable() {
                ...
                public void initialize() throws HyracksDataException {
                    ...
                    try {
                        ...
                        LOGGER.info("[FindCandidates] partition={} sampleTuples={} (bottom-up)", partition,
                                totalTupleCount);

                        BottomUpClusterStructure clusterStructure =
                        ...
                        if (clusterStructure.getNumLevels() == 0) {
                            LOGGER.warn(
                                    "[FindCandidates] partition={}: clustering produced 0 levels (sampleTuples={}); "
                                            + "emitting 0 tuples — VCTree will fail",
                                    partition, totalTupleCount);
                            return;
                        }

                        int totalCentroids = 0;
                        for (Integer lvl : clusterStructure.getLevelCentroids().keySet()) {
                            List<BottomUpClusterStructure.CentroidInfo> infos =
                                    clusterStructure.getLevelCentroids().get(lvl);
                            if (infos != null) {
                                totalCentroids += infos.size();
                            }
                        }
                        LOGGER.info("[FindCandidates] partition={}: emitting structure levels={} totalCentroids={}",
                                partition, clusterStructure.getNumLevels(), totalCentroids);
                        ...
                        LOGGER.error("[FindCandidates] partition={} failed during structure build", partition, e);
                        writer.fail();
                        throw HyracksDataException.create(
                                new RuntimeException("Hierarchical clustering failed on partition=" + partition, e));
                    } finally {
                        in.close();
                        writer.close();
                    }
                 ...
                 */
                private BottomUpClusterStructure performMemoryEfficientHierarchicalKMeans(IHyracksTaskContext ctx,
                        GeneratedRunFileReader in, FrameTupleAccessor fta, FrameTupleReference tuple,
                        IScalarEvaluator eval, IPointable inputVal, ListAccessor listAccessorConstant,
                        KMeansUtils kMeansUtils, VSizeFrame vSizeFrame, int partition, int totalTupleCount)
                        throws HyracksDataException, IOException {

                    BottomUpClusterStructure structure = new BottomUpClusterStructure();
                    ...
                    List<BottomUpClusterStructure.CentroidInfo> level0Info = new ArrayList<>();
                    for (int i = 0; i < initialResult.centroids.size(); i++) {
                        level0Info.add(
                                new BottomUpClusterStructure.CentroidInfo(i, -1, initialResult.centroids.get(i), 0));
                    }
                    structure.getLevelCentroids().put(0, level0Info);
                    ...
                    while (currentCentroids.size() > 1 && currentK > 1 && currentLevel < maxLevels) {
                        ...
                        if (BottomUpClusterStructure.doesLevelFitInFrame(levelResult.centroids.size(),
