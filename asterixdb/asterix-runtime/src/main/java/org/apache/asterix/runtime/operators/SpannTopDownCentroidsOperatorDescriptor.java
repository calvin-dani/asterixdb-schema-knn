...
public final class SpannTopDownCentroidsOperatorDescriptor extends AbstractOperatorDescriptor {
    ...
    private static final class TopDownClusterStructure {
        ...
        void outputBottomUpForStaticStructure(FrameTupleAppender appender, IFrameWriter writer, IHyracksTaskContext ctx)
            ...
            // Bottom-up: deepest treeLevel first, root last — matches VTreeStaticStructureBuilder.
            for (int level = maxLevel; level >= 0; level--) {
                ...
                List<Integer> idsAtLevel = levelGlobalIds.get(level);
                List<Integer> indices = new ArrayList<>(levelInfo.size());
                for (int i = 0; i < levelInfo.size(); i++) {
                    indices.add(i);
                }
                indices.sort(
                        (a, b) -> Integer.compare(levelInfo.get(a).parentClusterId, levelInfo.get(b).parentClusterId));
                for (int i : indices) {
    ...
    private static class EuclideanDistanceFunctionDouble implements IVTreeDistanceFunction, Serializable {
        private static final long serialVersionUID = 1L;
    ...
    private static class EuclideanSquaredDistanceFunctionDouble implements IVTreeDistanceFunction, Serializable {
        private static final long serialVersionUID = 1L;
    ...
    private static class CosineDistanceFunctionDouble implements IVTreeDistanceFunction, Serializable {
        private static final long serialVersionUID = 1L;
    ...
    private static class DotProductDistanceFunctionDouble implements IVTreeDistanceFunction, Serializable {
        private static final long serialVersionUID = 1L;
    ...
    /** Base seed for the training RNGs; per-partition (and per-phase) offsets keep the streams decorrelated. */
    private final long trainSeed;
    ...
    public SpannTopDownCentroidsOperatorDescriptor(IOperatorDescriptorRegistry spec, RecordDescriptor outputRecDesc,
            ...
            int bktLeafSizeOverride, long trainSeed) {
        super(spec, 1, 1);
        ...
        // Distance function from index DDL (WITH similarity); default l2_squared for training.
        this.distanceMetric = (distanceMetric == null || distanceMetric.trim().isEmpty()) ? "l2_squared"
                : distanceMetric.trim().toLowerCase(Locale.ROOT);
        this.distanceFunction = getDistanceFunctionDouble(this.distanceMetric);
    }
    ...
    protected class FindCandidatesActivity extends AbstractActivityNode {
        ...
        public IOperatorNodePushable createPushRuntime(final IHyracksTaskContext ctx,
            ...
            return new AbstractUnaryOutputSourceOperatorNodePushable() {
                ...
                @Override
                @AiProvenance(agent = AiProvenance.Agent.CLAUDE_FABLE_5, tool = AiProvenance.Tool.CLAUDE_CODE_UI, contributionKind = AiProvenance.ContributionKind.ASSISTED, notes = "Removed a never-read sample reader; delete the materialized sample run file once all phases finish")
                public void initialize() throws HyracksDataException {
                    // Materialized sample from Activity 1; every phase below opens its own reader on it.
                    ...
                    try {
                        ...
                        // Flatten branches deeper than global modal depth, then emit bottom-up (leaf-first) for
                        ...
                        // (SelectHead, head materialization, and the top-down build each open their own
                        // readers on it, all of which have finished by this point.)
                ...
                @AiProvenance(agent = AiProvenance.Agent.CLAUDE_FABLE_5, tool = AiProvenance.Tool.CLAUDE_CODE_UI, contributionKind = AiProvenance.ContributionKind.ASSISTED, notes = "Seeded the SelectHead RNG from trainSeed + partition for reproducible head selection")
                private HeadSelectionTaskState runSelectHeadPhase(IHyracksTaskContext ctx,
                    ...
                    // sample = same heads across runs (compiler.vector.trainseed).
                 ...
                 */
                @AiProvenance(agent = AiProvenance.Agent.CLAUDE_FABLE_5, tool = AiProvenance.Tool.CLAUDE_CODE_UI, contributionKind = AiProvenance.ContributionKind.ASSISTED, notes = "Seeded the top-down k-means RNG from trainSeed + partition (phase-offset) for reproducible builds")
                private TopDownClusterStructure buildTopDownHierarchicalKMeans(IHyracksTaskContext ctx,
                    ...
                    // Seeded from the descriptor's trainSeed with a per-partition offset; the extra constant keeps
                    // this phase's stream distinct from the SelectHead RNG on the same partition.
                    Random rand = new Random(trainSeed * 31 + partition + 1_000_003);
