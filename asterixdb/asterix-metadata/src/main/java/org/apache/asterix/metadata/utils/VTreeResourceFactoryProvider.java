    ...
    public IResourceFactory getResourceFactory(MetadataProvider mdProvider, Dataset dataset, Index index,
        ...
        if (dataset.getDatasetType() == DatasetType.INTERNAL) {
            ...
            if (withObjectNode != null) {
                int crossPollinationM = Math.max(1, withObjectNode.getOptionalInt("cross_pollination_m", 1));
                if (crossPollinationM > 1) {
                    double rngFactor = withObjectNode.getOptionalDouble("rng_factor", 1.0);
                    double levelwiseEpsilon = withObjectNode.getOptionalDouble("epsilon", 0.3);
