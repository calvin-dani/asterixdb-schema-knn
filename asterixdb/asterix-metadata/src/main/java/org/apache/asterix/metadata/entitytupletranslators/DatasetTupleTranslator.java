    ...
    protected Dataset createMetadataEntityFromARecord(ARecord datasetRecord) throws AlgebricksException {
        ...
        switch (datasetType) {
            case INTERNAL: {
                ...
                // Check if there is a randomSamplable field (defaults to false for old datasets)
                boolean randomSamplable = false;
                int randomSamplablePos = datasetDetailsRecord.getType()
                        .getFieldIndex(InternalDatasetDetails.RANDOM_SAMPLABLE_FIELD_NAME);
                if (randomSamplablePos >= 0) {
                    randomSamplable = ((ABoolean) datasetDetailsRecord.getValueByPos(randomSamplablePos)).getBoolean();
                }
                ...
                datasetDetails = new InternalDatasetDetails(fileStructure, partitioningStrategy, partitioningKey,
                        ...
                        filterField, isDatasetWithoutTypeSpec, randomSamplable);
                break;
            }
