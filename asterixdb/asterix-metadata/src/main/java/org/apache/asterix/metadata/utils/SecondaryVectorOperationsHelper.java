...
public class SecondaryVectorOperationsHelper extends SecondaryTreeIndexOperationsHelper {
    ...
    private static final int FULL_SCAN_THRESHOLD = 10000;
    ...
    public JobSpecification buildStaticStructureJobSpec() throws AlgebricksException {
        ...
        // This caps sampling to a minimum of 10,000, while still allowing a full scan if dataset cardinality < 10,000.
        if (datasetCardinality > 0) {
            sampleSize = (int) Math.min(sampleSize, datasetCardinality);
        }
        sampleSize = Math.max(sampleSize, 10000);
        sampleSize = Math.min(sampleSize, 1000000);
        if (datasetCardinality > 0) {
            sampleSize = (int) Math.min(sampleSize, datasetCardinality);
        }

        boolean useFullScan = sampleSize < FULL_SCAN_THRESHOLD;
     ...
     * Dataset dataset, Index vectorIndex, MetadataProvider,
    ...
    public JobSpecification buildQuantizationMetadataJobSpec() throws AlgebricksException {
        ...
        AdmObjectNode withObjectNode = indexDetails.getWithObjectNode();
        String quantization = (withObjectNode != null) ? withObjectNode.getOptionalString("quantization", null) : null;
        String qLabel = resolveEffectiveQuantizationLabel(quantization);
