...
public class PushFilterIntoVectorSearchRule implements IAlgebraicRewriteRule {
    ...
    @Override
    @AiProvenance(agent = AiProvenance.Agent.CLAUDE_FABLE_5, tool = AiProvenance.Tool.CLAUDE_CODE_UI, contributionKind = AiProvenance.ContributionKind.ASSISTED, notes = "Composite-PK fix: INCLUDE-field physical index now accounts for all primary key columns")
     ...
     */
    @AiProvenance(agent = AiProvenance.Agent.CLAUDE_FABLE_5, tool = AiProvenance.Tool.CLAUDE_CODE_UI, contributionKind = AiProvenance.ContributionKind.ASSISTED, notes = "Composite-PK fix: expose the dataset's primary key count for INCLUDE-field offset math")
    private VectorSearchInfo buildSearchInfo(UnnestMapOperator unnest, AccessMethodJobGenParams params,
        ...
        info.numPrimaryKeys = dataset.getPrimaryKeys().size();
