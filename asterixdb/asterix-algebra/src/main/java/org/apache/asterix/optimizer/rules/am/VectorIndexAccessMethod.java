...
public class VectorIndexAccessMethod implements IAccessMethod {
    ...
    // VECTOR_DISTANCE_ARRAY (index-driven KNN) is recognized in analyzeFuncExprArgsAndUpdateAnalysisCtx
    // but not yet wired into the optimization rule.
    ...
    public boolean analyzeFuncExprArgsAndUpdateAnalysisCtx(AbstractFunctionCallExpression funcExpr,
        ...
        } else if (fid.equals(BuiltinFunctions.EUCLIDEAN_DISTANCE)
                || fid.equals(BuiltinFunctions.EUCLIDEAN_SQUARED_DISTANCE)
                || fid.equals(BuiltinFunctions.COSINE_DISTANCE) || fid.equals(BuiltinFunctions.DOT_PRODUCT)) {
    ...
    public ILogicalOperator createIndexSearchPlan(Mutable<ILogicalOperator> limitRef,
        ...
        boolean isVectorDistance = annDistanceExpr.getFunctionIdentifier().equals(BuiltinFunctions.EUCLIDEAN_DISTANCE)
                || annDistanceExpr.getFunctionIdentifier().equals(BuiltinFunctions.EUCLIDEAN_SQUARED_DISTANCE)
                || annDistanceExpr.getFunctionIdentifier().equals(BuiltinFunctions.COSINE_DISTANCE)
                || annDistanceExpr.getFunctionIdentifier().equals(BuiltinFunctions.DOT_PRODUCT);
    ...
    public boolean exprIsOptimizable(Index index, IOptimizableFuncExpr optFuncExpr, boolean checkApplicableOnly,
        ...
        if (optFuncExpr.getFuncExpr().getFunctionIdentifier().equals(BuiltinFunctions.EUCLIDEAN_DISTANCE)
                || optFuncExpr.getFuncExpr().getFunctionIdentifier().equals(BuiltinFunctions.EUCLIDEAN_SQUARED_DISTANCE)
                || optFuncExpr.getFuncExpr().getFunctionIdentifier().equals(BuiltinFunctions.COSINE_DISTANCE)
                || optFuncExpr.getFuncExpr().getFunctionIdentifier().equals(BuiltinFunctions.DOT_PRODUCT)) {
    ...
    private boolean addIncludeFieldsAndSetSelectCondition(UnnestMapOperator vectorUnnestMap, SelectOperator selectOp,
        ...
        // so the condition references field-access expressions rather than ASSIGN-defined variables.
        ...
        //
        // Future solution: pass filter info through VectorJobGenParams and create TupleFilter
        // directly at the physical layer, bypassing the Algebricks type system.
        return false;
    }
