...
public class VectorIndexAccessMethod implements IAccessMethod {
    ...
    public boolean analyzeFuncExprArgsAndUpdateAnalysisCtx(AbstractFunctionCallExpression funcExpr,
        ...
        // Only ANN_DISTANCE is registered as optimizable (see FUNC_IDENTIFIERS); reject anything else.
    ...
    public ILogicalOperator createIndexSearchPlan(Mutable<ILogicalOperator> limitRef,
        ...
        // INCLUDE-field filter pushdown happens later, in PushFilterIntoVectorSearchRule (a physical
        // rewrite). It is intentionally NOT done here: the field-access predicate references the record
        // variable, which is not in this unnest's input type environment. Register the variables the
        // vector index search produces and move on.
