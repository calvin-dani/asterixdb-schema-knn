...
public class VectorIndexAccessMethod implements IAccessMethod {
    ...
    public boolean analyzeFuncExprArgsAndUpdateAnalysisCtx(AbstractFunctionCallExpression funcExpr,
        ...
        if (fid.equals(BuiltinFunctions.ANN_DISTANCE)) {
            if (funcExpr.getArguments().size() < 3 || funcExpr.getArguments().size() > 5) {
                ...
                || fid.equals(BuiltinFunctions.EUCLIDEAN_SQ_DISTANCE_ARRAY)
                || fid.equals(BuiltinFunctions.COSINE_DISTANCE_ARRAY)
                || fid.equals(BuiltinFunctions.DOT_DISTANCE_ARRAY)) {
    ...
    public ILogicalOperator createIndexSearchPlan(Mutable<ILogicalOperator> limitRef,
        ...
        boolean isVectorDistance =
                annDistanceExpr.getFunctionIdentifier().equals(BuiltinFunctions.EUCLIDEAN_DISTANCE_ARRAY)
                        || annDistanceExpr.getFunctionIdentifier().equals(BuiltinFunctions.EUCLIDEAN_SQ_DISTANCE_ARRAY)
                        || annDistanceExpr.getFunctionIdentifier().equals(BuiltinFunctions.COSINE_DISTANCE_ARRAY)
                        || annDistanceExpr.getFunctionIdentifier().equals(BuiltinFunctions.DOT_DISTANCE_ARRAY);
        ...
        // For now, always do a primary index lookup to get the full record.
    ...
    public boolean exprIsOptimizable(Index index, IOptimizableFuncExpr optFuncExpr, boolean checkApplicableOnly,
        ...
        if (optFuncExpr.getFuncExpr().getFunctionIdentifier().equals(BuiltinFunctions.EUCLIDEAN_DISTANCE_ARRAY)
                || optFuncExpr.getFuncExpr().getFunctionIdentifier()
                        .equals(BuiltinFunctions.EUCLIDEAN_SQ_DISTANCE_ARRAY)
                || optFuncExpr.getFuncExpr().getFunctionIdentifier().equals(BuiltinFunctions.COSINE_DISTANCE_ARRAY)
                || optFuncExpr.getFuncExpr().getFunctionIdentifier().equals(BuiltinFunctions.DOT_DISTANCE_ARRAY)) {
