...
/**
 * Marker interface for vector ANN search cursors that can expose the per-candidate distance
 ...
 *
 * <p>A cursor that doesn't actually compute a per-candidate query distance (for example the
 * streaming non-pruned {@code LSMVTreeSearchCursor}, which only sorts by distance-to-centroid)
 * should return {@link Double#NaN} so callers can detect the unsupported case.
 */
public interface IVectorSearchCursor {

    /**
     * @return the distance from the query vector to the candidate vector for the current tuple,
     *         or {@link Double#NaN} if the cursor does not maintain a per-tuple query distance.
     */
