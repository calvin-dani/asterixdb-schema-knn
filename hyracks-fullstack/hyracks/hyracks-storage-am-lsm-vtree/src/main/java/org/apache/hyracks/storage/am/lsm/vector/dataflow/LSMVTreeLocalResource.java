...
import org.apache.hyracks.storage.am.vector.impls.QuantizedVTreeDataTupleCreatorFactory;
...
public class LSMVTreeLocalResource extends LsmResource implements IQuantizedResource {
    ...
    private void tryReadQuantizationSidecarFile(IIOManager ioManager, FileReference indexFileRef) {
        try {
            ...
            LOGGER.log(Level.TRACE,
                    "[LSMVTreeLocalResource] Looking for sidecar file: " + sidecarFile.getAbsolutePath());

            if (!ioManager.exists(sidecarFile)) {
                LOGGER.log(Level.TRACE,
                        "[LSMVTreeLocalResource] Sidecar file not found, index will be created without quantization");
                return;
            }
            ...
            LOGGER.log(Level.TRACE,
                    "[LSMVTreeLocalResource] Read quantization constants from sidecar file: " + "minQ=" + minQuantile
            ...
            try {
                ...
                LOGGER.log(Level.TRACE,
                        "[LSMVTreeLocalResource] Deleted sidecar file: " + sidecarFile.getAbsolutePath());
            } catch (Exception e) {
                LOGGER.log(Level.TRACE, "[LSMVTreeLocalResource] Failed to delete sidecar file: " + e.getMessage());
            }

        } catch (IOException e) {
            LOGGER.log(Level.TRACE, "[LSMVTreeLocalResource] Error reading sidecar file: " + e.getMessage());
    ...
    public static IJsonSerializable fromJson(IPersistedResourceRegistry registry, JsonNode json)
        ...
        IVTreeDataTupleCreatorFactory dataTupleCreatorFactory;
        if (isQuantized) {
            dataTupleCreatorFactory = new QuantizedVTreeDataTupleCreatorFactory(numIncludeFields);
        } else {
            dataTupleCreatorFactory = new VTreeDataTupleCreatorFactory(numIncludeFields);
        }
