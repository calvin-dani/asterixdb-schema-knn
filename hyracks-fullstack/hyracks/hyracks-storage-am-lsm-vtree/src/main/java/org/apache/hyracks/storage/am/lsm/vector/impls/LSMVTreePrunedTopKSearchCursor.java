...
import org.apache.hyracks.storage.am.vector.impls.VTreeBidirectionCursor;
 ...
 *
 * Antimatter reconciliation follows LSMVTreeSearchCursor pattern:
...
public class LSMVTreePrunedTopKSearchCursor implements IIndexCursor {
    ...
    private VTreeBidirectionCursor[] vcbCursors;
    ...
    private VTreeSearchCursor[] resolverCursors;

    // Cursor state
    private boolean isOpen;
    private ResultEntry currentResult;

    // Multi-cluster probing state (following LSMVTreeSearchCursor pattern)
    ...
    public void open(ICursorInitialState initialState, ISearchPredicate searchPred) throws HyracksDataException {
        ...
        vcbCursors = new VTreeBidirectionCursor[numComponents];
        vTreeAccessors = new VTreeAccessor[numComponents];
        resolverCursors = new VTreeSearchCursor[numComponents];

        for (int i = 0; i < numComponents; i++) {
            ILSMComponent component = operationalComponents.get(i);
            VTree vTree = (VTree) component.getIndex();
            vTreeAccessors[i] = (VTreeAccessor) vTree.createAccessor(iap);
            vcbCursors[i] = (VTreeBidirectionCursor) vTreeAccessors[i].createBidirectionCursor();
            ...
            resolverCursors[i] = (VTreeSearchCursor) vTreeAccessors[i].createSearchCursor(false);
        }

        // Following LSMVTreeSearchCursor pattern:
        // Create a VTreeSearchCursor for first component to extract query vector and distance function
        if (numComponents > 0) {
            ...
            this.firstSearchCursor = (VTreeSearchCursor) vTreeAccessors[0].createSearchCursor(false);
            vTreeAccessors[0].search(firstSearchCursor, searchPred);

            // Get query vector and distance function from first cursor (like LSMVTreeSearchCursor does)
            ...
            // Set first cursor for DFS fallback (like LSMVTreeSearchCursor does)
        ...
        // Multi-cluster probing loop (following LSMVTreeSearchCursor pattern)
     ...
     * Follows LSMVTreeSearchCursor.advanceAllComponentsToSameCluster() pattern:
     ...
     * Follows LSMVTreeSearchCursor.compare() pattern:
     ...
     * Uses IVTreeBinaryAccessor to extract the vector from the tuple.
     *
