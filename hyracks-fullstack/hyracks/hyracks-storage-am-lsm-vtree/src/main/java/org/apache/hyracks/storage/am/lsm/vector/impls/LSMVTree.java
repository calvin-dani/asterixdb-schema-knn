...
public class LSMVTree extends AbstractLSMIndex implements ITreeIndex {
    ...
    // Data tuple format depends on quantization (see VTreeDataTupleCreator):
    ...
    public LSMVTree(NCConfig storageConfig, IIOManager ioManager, List<IVirtualBufferCache> virtualBufferCaches,
            ...
            boolean atomic, IVTreeBinaryAccessorFactory vectorAccessorFactory, int numPrimaryKeyFields,
            int numIncludeFields, IVTreeDataTupleCreatorFactory dataTupleCreatorFactory, float[] quantizationParams,
            String distanceMetric, IVTreeDistanceFunctionFactory distanceFunctionFactory,
        ...
        for (IVirtualBufferCache virtualBufferCache : virtualBufferCaches) {
            ...
            VTree vTree = new VTree(virtualBufferCache, new VirtualFreePageManager(virtualBufferCache),
                    ...
                    quantizationParams, this.distanceMetric, this.distanceFunctionFactory, this.crossPollination);
    ...
    public IIndexBulkLoader createBulkLoader(float fillFactor, boolean verifyInput, long numElementsHint,
        ...
        boolean isStaticStructureLoad = parameters.containsKey(LSMVTreeDiskComponent.PARAM_NUM_LEVELS)
                && parameters.containsKey(LSMVTreeDiskComponent.PARAM_CLUSTERS_PER_LEVEL)
                && parameters.containsKey(LSMVTreeDiskComponent.PARAM_CENTROIDS_PER_CLUSTER);
    ...
    public boolean isQuantized() {
        return quantizationParams != null || dataTupleCreatorFactory.isQuantized();
    }
    ...
    @Override
    @AiProvenance(agent = AiProvenance.Agent.CLAUDE_FABLE_5, tool = AiProvenance.Tool.CLAUDE_CODE_UI, contributionKind = AiProvenance.ContributionKind.ASSISTED, notes = "Fix: merge predicate now carries the layout-correct pkStartField")
    public ILSMDiskComponent doMerge(ILSMIOOperation operation) throws HyracksDataException {
        ...
        try {
            try {
                ...
                // The merge cursor's reconciliation key is <distance (field 0), PK...>. For
                // quantized layouts the PKs start at field 4, not the predicate's default 2;
                // leaving the default would pull quantized_distance/quantized_embedding into the
                // key, and field 2 has different write semantics between bulk load
                // (quantized-space) and DML (full-precision copy) — matter/antimatter twins would
                // fail to cancel during COMPACT.
