...
public class LSMVTree extends AbstractLSMIndex implements ITreeIndex {
    ...
    public IIndexBulkLoader createBulkLoader(float fillFactor, boolean verifyInput, long numElementsHint,
        ...
        boolean isStaticStructureLoad =
                parameters != null && parameters.containsKey(LSMVTreeDiskComponent.PARAM_NUM_LEVELS)
    ...
    /** Sum of component sizes across the merging components, used to estimate bulk loader size. */
    private long getNumberOfElements(List<ILSMComponent> mergedComponents) {
        long numElements = 0L;
        for (ILSMComponent comp : mergedComponents) {
            numElements += ((LSMVTreeDiskComponent) comp).getComponentSize();
        }
        return numElements;
    }
    ...
    public synchronized void deactivate(boolean flush) throws HyracksDataException {
        ...
        if (staticStructure != null) {
            try {
                ...
                // Best-effort cleanup; do not fail deactivation if static structure cleanup fails
                LOGGER.trace("Failed to deactivate static structure component: {}", e.getMessage());
            }
    ...
    public synchronized void destroy() throws HyracksDataException {
        ...
        if (staticStructure != null) {
            try {
                ...
                // Best-effort cleanup; do not fail destruction if static structure cleanup fails
                LOGGER.trace("Failed to destroy static structure component: {}", e.getMessage());
            }
            staticStructure = null;
        }
        super.destroy();
