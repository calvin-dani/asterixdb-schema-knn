...
public class VectorSearchOperatorNodePushable extends IndexSearchOperatorNodePushable {
    ...
    public VectorSearchOperatorNodePushable(IHyracksTaskContext ctx, int partition, RecordDescriptor inputRecDesc,
            ...
            int numSecondaryKeys, int kMultiplier, double indexEpsilon, int numPrimaryKeys, boolean indexOnly)
    ...
    private String extractDistanceMetricFromTuple(ITupleReference tuple, int fieldIndex) {
        // Only a genuinely absent metric field defaults to euclidean. A decode failure on a PRESENT field
        // must NOT be swallowed — doing so silently returns euclidean neighbors for e.g. a cosine query
        // (a silent wrong-results bug), so let it propagate.
        ...
        // Skip the 1-byte ADM type tag; the string window is the remaining (fieldLength - 1) bytes.
        stringPointable.set(fieldData, fieldStart + 1, fieldLength - 1);
     ...
     *       algebra-side variable type ({@code BuiltinType.ADOUBLE}). If the active cursor doesn't
     *       expose a query distance (i.e. the streaming {@code LSMVTreeSearchCursor}), we fail loudly
     *       — the optimizer should not have selected the index-only branch in that configuration.</li>
     * </ol>
     *
     * <p>In index-only mode the distance is read and validated <em>before</em> the projector runs, so a
     * missing/NaN {@code D(q,x)} throws without leaving a half-written tuple in the shared {@code tb}
     * (the projector already wrote the PK fields). The two failures the guard covers — a non-vector
     * cursor and a NaN distance from a vector cursor — are both invariant enough to check up front:
     * neither depends on {@code project()} having run, and {@code getCurrentDistance()} is valid for the
     * current tuple both before and after projection (projection does not advance the cursor).
     */
    @Override
    protected ITupleReference writeTupleToOutput(ITupleReference tuple) throws IOException {
        double dqx = Double.NaN;
        if (indexOnly) {
            if (!(activeCursor instanceof IVectorSearchCursor)) {
                throw new IOException("index-only ANN plan requires a vector cursor exposing per-tuple "
                        + "D(q,x); the current cursor type does not (likely the streaming "
                        + "LSMVTreeSearchCursor — use the pruned top-K cursor instead).");
            }
            dqx = ((IVectorSearchCursor) activeCursor).getCurrentDistance();
            if (Double.isNaN(dqx)) {
                throw new IOException("index-only ANN plan read a NaN D(q,x) from the vector cursor for the "
                        + "current candidate; the cursor did not expose a per-tuple query distance.");
            }
        }
