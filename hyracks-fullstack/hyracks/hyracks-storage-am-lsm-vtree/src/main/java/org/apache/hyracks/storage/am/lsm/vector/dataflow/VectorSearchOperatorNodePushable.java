...
public class VectorSearchOperatorNodePushable extends IndexSearchOperatorNodePushable {
    ...
    // Search approach (compile-time constant from descriptor; see addAdditionalIndexAccessorParams).
    // 0 = naive streaming (LSMVTreeSearchCursor); 3 = top-K with quantized distance.
    ...
    protected int getFieldCount(IIndex index) {
        // Vector index search only outputs primary keys (no secondary keys / embeddings).
        // Currently assumes a single PK field (common case); revisit if composite PKs are supported.
        return 1;
    }
