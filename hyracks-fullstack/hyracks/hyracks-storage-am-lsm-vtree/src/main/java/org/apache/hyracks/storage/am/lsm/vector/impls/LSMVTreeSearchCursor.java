...
public class LSMVTreeSearchCursor extends LSMIndexSearchCursor {
    ...
    public void doOpen(ICursorInitialState initialState, ISearchPredicate searchPred) throws HyracksDataException {
        ...
        currentClusterIndex = new int[numVTrees];
        ...
        LOGGER.log(Level.TRACE,
                String.format("[LSMVTreeSearchCursor] doOpen: numComponents=%d, K=%d, nprobe=%d, pkStartField=%d",
                        numVTrees, K, nprobe, pkStartField));
        ...
        if (numVTrees > 0) {
            ...
            if (firstCluster != null) {
                ...
                if (dfsCluster != null && dfsCluster.centroidId != firstCluster.centroidId) {
                    ...
                    LOGGER.log(Level.TRACE,
                            String.format(
                                    "[LSMVTreeSearchCursor] DFS found cid=%d but level-wise[0] is cid=%d - re-opening all cursors to level-wise[0]",
                ...
                LOGGER.log(Level.TRACE,
                        String.format(
                                "[LSMVTreeSearchCursor] Computed %d level-wise clusters, first cluster cid=%d marked visited",
                ...
                if (dfsFallbackCluster != null) {
                    ...
                    LOGGER.log(Level.TRACE,
                            String.format(
                                    "[LSMVTreeSearchCursor] Level-wise disabled, marked first DFS cluster as visited: cid=%d",
    ...
    public void initPriorityQueue() throws HyracksDataException {
        ...
        for (int i = 0; i < rangeCursors.length; i++) {
            if (rangeCursors[i].hasNext()) {
                ...
                LOGGER.log(Level.TRACE, String
                        .format("[LSMVTreeSearchCursor] Component %d has empty initial cluster (marked exhausted)", i));
            }
        }
        ...
        if (allInitiallyExhausted) {
            LOGGER.log(Level.TRACE,
                    "[LSMVTreeSearchCursor] ALL components have empty initial cluster, advancing to next");
        ...
        for (int i = replaceFrom; i < switchRequest.length && i < operationalComponents.size(); i++) {
            if (switchRequest[i]) {
                ...
                if (switchedElements[i] != null) {
                    rangeCursors[i].close();
                    vTreeAccessors[i] = (VTree.VTreeAccessor) vTree.createAccessor(iap);
    ...
    private void pushIntoQueueAndAdvanceClusterIfNeeded(PriorityQueueElement e) throws HyracksDataException {
        ...
        if (clusterStrategy.shouldStopAdvancing(minClustersExplored, resultsCollected)) {
            ...
            stopAdvancing = true;
            LOGGER.log(Level.TRACE,
                    String.format("[LSMVTreeSearchCursor] Early termination: minClustersExplored=%d, returned=%d",
    ...
    private void advanceAllComponentsToSameCluster() throws HyracksDataException {
        ...
        while (true) {
            ...
            if (nextCluster == null) {
                LOGGER.log(Level.TRACE, "[LSMVTreeSearchCursor] No more clusters available globally");
                for (int i = 0; i < clusterExhausted.length; i++) {
                    clusterExhausted[i] = true;
                }
                return;
            ...
            LOGGER.log(Level.TRACE,
                    String.format("[LSMVTreeSearchCursor] Global cluster selected: cid=%d, distance=%.4f, dirPage=%d",
