 ...
 */
public class LSMVTreeSearchCursor extends LSMIndexSearchCursor implements IVectorSearchCursor {
    ...
    public void doOpen(ICursorInitialState initialState, ISearchPredicate searchPred) throws HyracksDataException {
        ...
        this.pkStartField = ((VTreeSearchPredicate) searchPred).getPkStartField();
    ...
    private class VectorPriorityQueueComparator extends PriorityQueueComparator {
        ...
        public int compare(PriorityQueueElement elementA, PriorityQueueElement elementB) {
            ...
            try {
                ...
                // Compare order: distance first, then the PK fields ONLY (secondary and INCLUDE
                // fields are excluded — see the numPrimaryKeyFields comment)
     ...
     * {@inheritDoc}
     *
     * <p>This streaming cursor only orders candidates by their stored distance-to-centroid and
     * never computes a per-tuple {@code D(q,x)}, so it returns {@link Double#NaN}. The index-only
     * ANN plan therefore requires the pruned top-K cursor, not this one.
     */
    @Override
    ...
    @Override
    @AiProvenance(agent = AiProvenance.Agent.CLAUDE_FABLE_5, tool = AiProvenance.Tool.CLAUDE_CODE_UI, contributionKind = AiProvenance.ContributionKind.ASSISTED, notes = "Fix: cancellation key capped to <distance, PK...> (excludes INCLUDE fields)")
    protected int compare(MultiComparator cmp, ITupleReference tupleA, ITupleReference tupleB)
            throws HyracksDataException {

        // Tuple format (non-quantized): [distance, centroidId, PKs..., includes...]
        // Tuple format (quantized): [distance, centroidId, quantized_distance, quantized_embedding, PKs..., includes...]
        // The cancellation key is exactly <distance, PK fields>: secondary fields (centroidId,
        // and for quantized layouts quantized_distance/quantized_embedding) and trailing INCLUDE
        // fields are excluded — they may differ between matter/antimatter twins.
