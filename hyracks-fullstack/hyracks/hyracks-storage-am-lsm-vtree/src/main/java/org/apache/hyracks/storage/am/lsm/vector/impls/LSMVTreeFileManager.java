...
public class LSMVTreeFileManager extends AbstractLSMIndexFileManager {
    ...
    protected String getNextComponentSequence(FilenameFilter filenameFilter) throws HyracksDataException {
        // For VTree, the static structure is implicitly component [0,0]
    ...
    public List<LSMComponentFileReferences> cleanupAndGetValidFiles() throws HyracksDataException {
        ...
        // Gather all VTree files
        validateFiles(baseDir.getFile(), vTreeFilter, allVTreeFiles, reported, vTreeFactory);
        ...
        Collections.sort(allVTreeFiles, Collections.reverseOrder());
        ...
        if (!validateStaticStructureFile(staticStructureFile)) {
            ...
            // Clean up all VTree files since they can't work without the static structure
        ...
        // Process each VTree file - all share the same static structure
        for (IndexComponentFileReference vTreeFile : allVTreeFiles) {
            String baseName = vTreeFile.getSequence();
            LOGGER.log(Level.TRACE, "Valid VTree component found: {} (using shared .staticstructure)", baseName);
