 ...
 */
public class LSMVTreeTopKSearchCursor implements IIndexCursor, IVectorSearchCursor {
    ...
    public void close() throws HyracksDataException {
        if (isOpen) {
            ...
            // Guard on the array itself: open() sets isOpen=true before allocating rangeCursors, so a
            // failure early in open() can leave it null while isOpen is true.
    ...
    public void destroy() throws HyracksDataException {
        close();
        // Release the per-component accessors + cursors for good — close() only ends the current search
        // (it can be re-opened); destroy() must reclaim them, matching LSMVTreeSearchCursor.
