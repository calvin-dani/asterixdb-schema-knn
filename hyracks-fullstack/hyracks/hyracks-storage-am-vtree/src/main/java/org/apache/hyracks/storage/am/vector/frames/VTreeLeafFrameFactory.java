...
public class VTreeLeafFrameFactory implements ITreeIndexFrameFactory {
     ...
     * neighbor_list : byte[], child_page_pointer : int>}.
     * <p>
     * The graph-neighbor list sits just before the child (metadata) page pointer so that the pointer
     * stays the last field — {@code getMetadataPagePointer()} reads {@code getFieldCount()-1} and
     * {@code getQuantizedCentroidBytes()} reads field index 2, both unaffected. The neighbor list is
     * always present (empty when a leaf has no neighbors).
     */
    private static final ITypeTraits[] TYPE_TRAITS_QUANTIZED = { IntegerPointable.TYPE_TRAITS, // cluster ID
            ...
            VarLengthTypeTrait.INSTANCE, // graph neighbor list (byte[])
