...
public class VTreeBulkLoader extends PageWriteFailureCallback implements IIndexBulkLoader {
    ...
    public void end() throws HyracksDataException {
        ...
        // Maps each leaf centroid id to its final placement in this data component: {pageId, slot}.
        // Built while copying leaf pages (final page id and slot are known at copy time), then used to
        // resolve graph-neighbor entries from provisional centroid ids to physical pointers.
        ...
        for (int i = 0; i < numStaticPages; i++) {
            ...
            if (level > 0) {
                ...
                for (int t = 0; t < leafTupleCount; t++) {
                    ...
                    // Record this leaf centroid's final location for neighbor resolution below.
                    cidToFinalLoc.put(centroidId, new int[] { newPageId, t });
                }
        ...
        // Translate graph-neighbor pointers into this component's page space. The static structure now
        // carries resolved pointers (resolved by the builder in the scaffold's own page space), so each
        // is offset by staticBasePageId — the same shift applied to child/next-leaf pointers above. Any
        // still-provisional entry (e.g. from an older scaffold) is resolved from the directory instead.
        resolveLeafNeighborPointers(staticPages, lfFrame, cidToFinalLoc, staticBasePageId);
    ...
    /**
     * Resolve each leaf page's graph-neighbor list from provisional form (neighbor centroid ids) to
     * resolved physical pointers ({pageId, slot} in this data component), using the directory built
     * during the static-page copy. Quantized leaf tuples carry the neighbor list at field index 3 (a
     * length-prefixed byte array of fixed-width entries); leaf layouts without a neighbor field are
     * skipped, as are entries whose neighbor centroid id is not in the directory (dangling).
     * <p>
     * Each page is rewritten in place (same byte width, so slots do not move) before it is written
     * exactly once by the caller — no page is re-uploaded, honoring the cloud append-only rule.
     */
    private void resolveLeafNeighborPointers(TreeMap<Integer, ICachedPage> staticPages, IVTreeLeafFrame lfFrame,
            Map<Integer, int[]> cidToFinalLoc, int staticBasePageId) throws HyracksDataException {
        for (ICachedPage page : staticPages.values()) {
            lfFrame.setPage(page);
            if (lfFrame.getLevel() != 0) {
                continue; // interior/root page: no neighbor lists
            }
            ...
            for (int t = 0; t < tupleCount; t++) {
                ...
                // Neighbor list sits at LEAF_NEIGHBOR_LIST_FIELD, before the last (metadata-pointer) field;
                // absent in leaf layouts with fewer fields (e.g. non-quantized fixtures).
                ...
                for (int e = 0; e < numEntries; e++) {
                    if (VTreeLeafNeighborList.isResolved(fieldData, contentStart, e)) {
                        ...
                        VTreeLeafNeighborList.writeResolved(fieldData, contentStart, e, pageId + staticBasePageId,
                                slot);
                    } else {
