...
public class VTreeBulkLoader extends PageWriteFailureCallback implements IIndexBulkLoader {
    ...
    private final IComponentSampler sampler;
     ...
     */
    @AiProvenance(agent = AiProvenance.Agent.CLAUDE_FABLE_5, tool = AiProvenance.Tool.CLAUDE_CODE_UI, contributionKind = AiProvenance.ContributionKind.ASSISTED, notes = "Fix: delete-marker polarity stripped on partial merge (delete resurrection); marker semantics in lsm-vtree")
    @AiProvenance(agent = AiProvenance.Agent.CLAUDE_FABLE_5, tool = AiProvenance.Tool.CLAUDE_CODE_UI, contributionKind = AiProvenance.ContributionKind.REFACTORED, notes = "Bounded-memory static-structure copy (review feedback): read source metadata only at init; page contents are streamed one page at a time in end()")
    public VTreeBulkLoader(IPageWriteCallback callback, VTree vectorTree, ITreeIndexAccessor staticAccessor,
            IComponentSampler sampler, ITreeIndexFrameFactory dataFrameFactoryOverride) throws HyracksDataException {
        ...
        MutableArrayValueReference key1 =
                new MutableArrayValueReference(VTreeMetadataKeys.NUM_LEAF_CENTROIDS.getBytes());
        LongPointable value1 = LongPointable.FACTORY.createPointable();
        MutableArrayValueReference key2 =
                new MutableArrayValueReference(VTreeMetadataKeys.FIRST_LEAF_CENTROID_ID.getBytes());
        ...
        for (int i = 0; i < numLeafCentroid; i++) {
            clusterFirstDirPageId[i] = VTreeDataTupleConstants.UNASSIGNED_DIR_PAGE;
        }
    ...
    private int extractCentroidId(ITupleReference tuple) {
        ...
        final int cidField = VTreeDataTupleConstants.NQ_CENTROID_ID_FIELD;
        return IntegerPointable.getInteger(tuple.getFieldData(cidField), tuple.getFieldStart(cidField));
    }

    @Override
    @AiProvenance(agent = AiProvenance.Agent.CLAUDE_OPUS_4_8, tool = AiProvenance.Tool.CLAUDE_CODE_UI, contributionKind = AiProvenance.ContributionKind.ASSISTED, notes = "Oversized-tuple guard (review feedback): throw RECORD_IS_TOO_LARGE before the overflow/new-page path when a tuple exceeds a fresh empty page's usable space, instead of looping/overrunning at insertSorted()")
    public void add(ITupleReference tuple) throws HyracksDataException {
        ...
        try {
            ...
            // Oversized-tuple guard (reviewer feedback, ASTERIXDB-3754): a tuple that cannot fit in an
            // otherwise-empty data page would loop forever here — finishCurrentDataPage() allocates a fresh
            // page and insertSorted() is then called WITHOUT re-checking that the tuple fits, silently
            // overrunning the page. Unlike BTree we do NOT use the (pageSize-header)/2 splittability bound:
            // VTree legitimately stores one large tuple per page via next-page overflow chaining, so the
            // real threshold is the usable space of a FRESH empty page = pageSize - pageHeaderSize - slotSize
            // (resetSpaceParams() sets totalFreeSpace = pageSize - pageHeaderSize on init, and each tuple
            // also consumes one slot). Check BEFORE the overflow/new-page logic so it fires regardless of
    ...
    private void finishCurrentDataPage(boolean lastPage) throws HyracksDataException {
        ...
        if (lastPage) {
            ...
            ((IVTreeDataFrame) currentDataFrame).setNextPage(VTreeDataTupleConstants.NO_NEXT_PAGE);
    ...
    private void finalizeClusterDirectory() throws HyracksDataException {
        ...
        for (int i = 0; i < numDirPages; i++) {
            ...
            if (i < numDirPages - 1) {
                ...
                ((IVTreeMetadataFrame) currentDirectoryFrame).setNextPage(VTreeDataTupleConstants.NO_NEXT_PAGE);
    ...
    @Override
    @AiProvenance(agent = AiProvenance.Agent.CLAUDE_FABLE_5, tool = AiProvenance.Tool.CLAUDE_CODE_UI, contributionKind = AiProvenance.ContributionKind.REFACTORED, notes = "Bounded-memory static-structure copy (review feedback): pass 1 builds an O(K) cid->location map, pass 2 streams one source/destination page pair at a time instead of confiscating all pages")
     ...
     */
    @AiProvenance(agent = AiProvenance.Agent.CLAUDE_FABLE_5, tool = AiProvenance.Tool.CLAUDE_CODE_UI, contributionKind = AiProvenance.ContributionKind.REFACTORED, notes = "Bounded-memory static-structure copy (review feedback): per-page resolution against a precomputed cid map instead of a whole-structure page sweep")
    private void resolveLeafNeighborPointers(IVTreeLeafFrame lfFrame, Map<Integer, int[]> cidToFinalLoc,
        ...
        VTreeLeafNeighborList.forEachLeafNeighborEntry(lfFrame, (fieldData, contentStart, e) -> {
            if (VTreeLeafNeighborList.isResolved(fieldData, contentStart, e)) {
                ...
                int neighborCid = VTreeLeafNeighborList.readCentroidId(fieldData, contentStart, e);
                int[] loc = cidToFinalLoc.get(neighborCid);
                if (loc != null) {
                    VTreeLeafNeighborList.writeResolved(fieldData, contentStart, e, loc[0], loc[1]);
                }
            }
        });
    }
