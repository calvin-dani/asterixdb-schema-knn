...
public class VTreeNavigationUtils {
    ...
    private static PageScan scanPage(IBufferCache bufferCache, int fileId, int pageId, double[] queryVector,
        ...
        try {
            ...
            if (leafFrame.isLeaf()) {
                return new PageScan(collectAllLeafCentroids(bufferCache, fileId, queryVector, pageId, leafFrame,
                        leafFrameFactory, distanceFunction, quantizedQueryVector, quantizer), null);
            }
            IVTreeInteriorFrame interiorFrame = (IVTreeInteriorFrame) interiorFrameFactory.createFrame();
            interiorFrame.setPage(page);
            return new PageScan(null, collectAllChildCentroids(bufferCache, fileId, queryVector, pageId, interiorFrame,
                    interiorFrameFactory, distanceFunction));
        } finally {
    ...
    public static ClusterSearchResult initializeClusterIterator(VTreeNavigationState state,
        ...
        while (true) {
            ...
            if (scan.isLeaf()) {
                ...
                VTreeNavigationFrame leafNavFrame = new VTreeNavigationFrame(currentPageId, scan.leafCentroids, true);
            ...
            VTreeNavigationFrame interiorNavFrame = new VTreeNavigationFrame(currentPageId, scan.children);
