...
public class VTreeNavigationUtils {
        ...
        while (true) {
            ...
            try {
                ...
                if (isLeaf) {
                    ...
                    VTreeNavigationFrame leafNavFrame = new VTreeNavigationFrame(currentPageId, sortedCentroids, true);
                    state.stack.push(leafNavFrame);

                    // Return first centroid as closest cluster, marking it visited
                    VTreeLeafCentroid first = leafNavFrame.nextCentroid();
                    ...
                    VTreeNavigationFrame interiorNavFrame = new VTreeNavigationFrame(currentPageId, sortedChildren);
                    state.stack.push(interiorNavFrame);

                    // Descend to closest child
                    VTreeChildCentroid closest = interiorNavFrame.nextChild();
    ...
    public static List<ClusterSearchResult> findCloseCentroidsLevelWiseGlobalSort(IBufferCache bufferCache, int fileId,
            ...
            for (VTreeLevelNode node : currentLevelNodes) {
                ...
                try {
                    ...
                    if (isLeaf) {
                        ...
                        double localThreshold = epsilonThreshold(closestDistance, epsilon);
        ...
        if (epsilon > 0.0) {
            ...
            double globalThreshold = epsilonThreshold(globalClosestDistance, epsilon);
