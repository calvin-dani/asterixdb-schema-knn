...
import org.apache.hyracks.storage.am.vector.utils.VTreeNavigationFrame;
...
public class VTreeSearchCursor implements IIndexCursor {
    ...
    private int extractNprobe(ISearchPredicate searchPred) {
        if (searchPred instanceof VTreeSearchPredicate) {
            return ((VTreeSearchPredicate) searchPred).getNprobe();
        }
    ...
    public void resetDfsFirstCentroid() {
        if (iteratorState != null && iteratorState.initialized && !iteratorState.stack.isEmpty()) {
            VTreeNavigationFrame topFrame = iteratorState.stack.peek();
     ...
     *
     * Used by LSMVTreeBlockedCursor to resolve per-component directory page IDs
