    ...
    public int copyStaticStructure(VTree.VTreeAccessor staticAccessor) throws HyracksDataException {
        ...
        for (int i = 0; i < numStaticPages; i++) {
            ...
            if (level > 0) {
                ...
                //
                // Fix bug-vtree-post-compact-recall: derive centroidDirPageMap index from the
                // slot's stored centroid_id, NOT from a running counter that walks pages in
                // physical-page-id order. The page-id order is not guaranteed to match the
                // nextLeaf chain order; the previous running-counter form assigned slot K's
                // pointer to centroidDirPageMap[K-th-slot-in-page-id-order], which scrambled the
                // mapping whenever physical page ids and nextLeaf chain order diverged. Reading
                // the slot's actual centroid_id and using (cid - firstLeafCid) as the index
                // makes the assignment robust to any traversal order.
