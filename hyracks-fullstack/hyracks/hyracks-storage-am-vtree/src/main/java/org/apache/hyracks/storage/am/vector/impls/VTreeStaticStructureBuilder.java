...
public class VTreeStaticStructureBuilder extends PageWriteFailureCallback implements IIndexBulkLoader {
    ...
    @Override
    @AiProvenance(agent = AiProvenance.Agent.CLAUDE_OPUS_4_8, tool = AiProvenance.Tool.CLAUDE_CODE_UI, contributionKind = AiProvenance.ContributionKind.ASSISTED, notes = "Oversized-tuple guard (review feedback): throw RECORD_IS_TOO_LARGE before createOverflowPage() when the ENTRY tuple (post child/metadata-pointer append) exceeds a fresh empty page's usable space, instead of looping/overrunning at insertSorted()")
    public void add(ITupleReference tuple) throws HyracksDataException {
        ...
        // Oversized-tuple guard (reviewer feedback, ASTERIXDB-3754): an entry tuple that cannot fit in an
        // otherwise-empty page would loop forever — createOverflowPage() opens a fresh page and insertSorted()
        // is then called WITHOUT re-checking that the tuple fits, silently overrunning the page. The guard is on
        // the ENTRY tuple (post-append: createEntryTuple appends the child/metadata pointer), since that is what
        // gets written. Unlike BTree we do NOT use the (pageSize-header)/2 splittability bound: the leaf/interior
        // static pages carry full-precision centroids and legitimately fill a page with few large tuples, so the
        // real threshold is the usable space of a FRESH empty page = pageSize - pageHeaderSize - slotSize
        // (resetSpaceParams() inits totalFreeSpace = pageSize - pageHeaderSize, and each tuple consumes one slot).
        // getPageHeaderSize() dispatches per current frame type (leaf vs interior). Check BEFORE createOverflowPage.
    ...
    private ITupleReference createEntryTuple(ITupleReference tuple, int childPageId) throws HyracksDataException {
        ...
        try {
            ArrayTupleBuilder tupleBuilder = new ArrayTupleBuilder(inputFieldCount + 1);
            for (int i = 0; i < inputFieldCount; i++) {
                tupleBuilder.addField(tuple.getFieldData(i), tuple.getFieldStart(i), tuple.getFieldLength(i));
            }
            tupleBuilder.getDataOutput().writeInt(childPageId);
            tupleBuilder.addFieldEndOffset();

            ArrayTupleReference entryTuple = new ArrayTupleReference();
            entryTuple.reset(tupleBuilder.getFieldEndOffsets(), tupleBuilder.getByteArray());
            return entryTuple;
        } catch (Exception e) {
    ...
    /**
     * Leaf resolution pass. Re-reads each leaf page (from local storage, where placement published it),
     * rewrites its provisional neighbor entries to physical pointers {@code (pageId, slot)} using the
     * directory, and uploads the page exactly once via {@link IBufferCache#flush}. Pages are processed
     * in ascending page-id order, so the append-only cloud writer sees monotonically increasing offsets
     * (and all leaf uploads precede any interior/root upload, exactly as in the single-pass build).
     * <p>
     * Assumes the static-structure file is not page-compressed — {@code flush()} does not run the
     * compression prepare step that the FIFO write path uses.
     */
    ...
    private void resolveLeafPageNeighbors(ICachedPage page) throws HyracksDataException {
        ...
        VTreeLeafNeighborList.forEachLeafNeighborEntry(leafFrame, (fieldData, contentStart, e) -> {
