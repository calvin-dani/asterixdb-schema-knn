...
public class VTree extends AbstractTreeIndex {
    ...
    private void insertVector(ITupleReference tuple, VTreeOpContext ctx) throws HyracksDataException {
        ClusterAccessResult accessResult = findClusterAndPrepareAccess(tuple, ctx);
    ...
    private void splitDataPageMaintainOrder(long dataPageId, ITupleReference newTuple, int insertIndex,
        ...
        try {
            ...
            // TODO: use proper LSN from LSM infrastructure instead of wall clock
            long currentLsn = System.currentTimeMillis();
     ...
     */
    private void updateMetadataAfterDataSplit(long originalDataPageId, int newDataPageId, VTreeOpContext ctx)
            throws HyracksDataException {
    ...
    private void deleteVector(ITupleReference tuple, VTreeOpContext ctx) throws HyracksDataException {
        ...
        try {
            ...
            boolean foundAndDeleted = tryPhysicalDelete(accessResult.metadataPageId, distance, primaryKey, tuple, ctx);
    ...
    private byte[] extractPrimaryKeyFromTuple(ITupleReference tuple) {
        return VTreeTupleUtils.extractPrimaryKeyFromTuple(tuple);
    }
    ...
    private double[] extractVectorFromTuple(ITupleReference tuple) throws HyracksDataException {
        ...
        IVTreeBinaryAccessor accessor = vectorAccessorFactory.createAccessor();
     ...
     * @return IVTreeDistanceFunction implementation, or Euclidean as default
     */
    private static IVTreeDistanceFunction convertDistanceMetricToFunction(String distanceMetric) {
        ...
        // Map distance metric strings to IVTreeDistanceFunction implementations
        switch (metricLower) {
            case "euclidean":
            case "l2":
                return VectorUtils::calculateEuclideanDistance;
     ...
     */
    public ClusterSearchResult findClosestClusterFromRoot(double[] queryVector, VTreeOpContext ctx,
    ...
    public List<ClusterSearchResult> findCloseCentroidsLevelWiseGlobalSortFromRoot(double[] queryVector,
            VTreeOpContext ctx, IVTreeDistanceFunction distanceFunction, double ep) throws HyracksDataException {
    ...
    private ClusterAccessResult findClusterAndPrepareAccess(ITupleReference tuple, VTreeOpContext ctx)
        ...
        if (centroidDirPageMap != null) {
            return new ClusterAccessResult(clusterResult, null, clusterResult.directoryPageId, true);
        }
        ...
        try {
            ...
            return new ClusterAccessResult(clusterResult, leafPage, metadataPageId, true);
        } catch (Exception e) {
            try {
    ...
    public class VTreeAccessor implements ITreeIndexAccessor {
        ...
        /** @deprecated Stub for backward compatibility. Calvin: remove callers and this method. */
        @Deprecated
        public List<ClusterSearchResult> findCloseLeafCentroid(double[] queryVector,
                IVTreeDistanceFunction hyracksDistanceFunction, double epi) {
