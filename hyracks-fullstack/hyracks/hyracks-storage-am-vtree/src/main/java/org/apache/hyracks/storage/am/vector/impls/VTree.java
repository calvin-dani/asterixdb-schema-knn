...
public class VTree extends AbstractTreeIndex {
    ...
    /** Distance metric assumed when the index was created without an explicit metric. */
    ...
    public VTree(IBufferCache bufferCache, IPageManager freePageManager, ITreeIndexFrameFactory interiorFrameFactory,
        ...
        this.distanceMetric = distanceMetric != null ? distanceMetric : DEFAULT_DISTANCE_METRIC;

        this.distanceFunctionFactory = distanceFunctionFactory;
        this.distanceFunction = distanceFunctionFactory.createDistanceFunction(this.distanceMetric);
    ...
    private boolean tryInsertIntoDataPage(long dataPageId, double[] vector, double distance, int centroidId,
        ...
        try {
            ...
            // Pass context so createDataTuple can check operation type and encode a delete-polarity tuple
                    ...
                    try {
                        ...
                        if (tupleIndex >= 0) {
                            ...
                            if (!Arrays.equals(pkAtIndex, primaryKey)) {
                                // findTupleByDistanceAndPrimaryKey returned a positive hit whose PK does
                                // not actually match: an internal inconsistency in the delete-reconciliation
                                // path. Returning false makes the caller write a delete marker (double-write),
                                // so surface it in logs rather than degrade silently. Control flow unchanged.
                                LOGGER.log(Level.WARN,
                                        "Delete PK mismatch: findTupleByDistanceAndPrimaryKey hit tuple {} on data page {} "
                                                + "(metadata page {}, distance {}) but its PK does not match the delete key; "
                                                + "downgrading to not-found (caller will write a delete marker)",
                                        tupleIndex, targetDataPageId, currentMetadataPageId, distance);
                                return false;
                            }
    ...
    private void updateMetadataWithNewDataPage(long metadataPageId, int newDataPageId, double maxDistance,
        ...
        ITupleReference metadataTuple = metadataFrame.createMetadataTuple(maxDistance, newDataPageId);
    ...
    private void handleMetadataPageOverflow(long metadataPageId, ITupleReference newTuple, VTreeOpContext ctx)
        ...
        try {
            ...
            rightFrame.setNextPage(VTreeDataTupleConstants.NO_NEXT_PAGE);
     ...
     * <p>
     * With cross-pollination disabled ({@code M == 1}) this is the single closest cluster — byte-for-byte
     * the legacy behavior. With {@code M > 1} it reproduces the bulk-load placement exactly: the
     * eps-filtered, RNG-thinned top-{@code M} accepted centroids
     ...
     * this resolves to the same set at insert time and at delete time — the property delete reconciliation
     * relies on to cancel every replica.
     */
    private List<ClusterSearchResult> findReplicaClusters(double[] vector) throws HyracksDataException {
        if (!crossPollination.enabled()) {
        ...
        if (accepted.isEmpty()) {
            ...
            // the single closest cluster (which is also the first replica bulk-load would have produced).
            return Collections.singletonList(requireCluster(findClosestClusterFromRoot(vector, distanceFunction)));
        }
        return accepted;
    }
    ...
    public class VTreeAccessor implements ITreeIndexAccessor {
        ...
        public void search(IIndexCursor cursor, ISearchPredicate searchPred) throws HyracksDataException {
            ...
            String distanceMetric = resolveDistanceMetric(searchPred);
            IVTreeDistanceFunction distanceFunction =
                    resolveDistanceFunctionFactory().createDistanceFunction(distanceMetric);

            VTreeCursorInitialState initialState = buildInitialState(queryVector, distanceFunction);
            resolveAndSetQuantizer(initialState, queryVector, distanceMetric);
        ...
        private double[] extractQueryVector(ISearchPredicate searchPred) throws HyracksDataException {
            if (searchPred instanceof VTreeSearchPredicate vectorPred && vectorPred.getQueryTuple() != null) {
                ...
                return VTreeTupleUtils.extractVectorFromTuple(vectorPred.getQueryTuple(),
                        vectorPred.getQueryFieldIndex(), factory);
            }
            return null;
        }

        /** Metric from the predicate, or the index's own metric for the metric-less full-scan predicate. */
        private String resolveDistanceMetric(ISearchPredicate searchPred) {
            if (searchPred instanceof VTreeSearchPredicate vectorPred && vectorPred.getDistanceMetric() != null) {
                return vectorPred.getDistanceMetric();
            }
            return tree.distanceMetric;
        }

        /**
         * Prefer a query-time factory from IAP (see
         * VectorSearchOperatorNodePushable#addAdditionalIndexAccessorParams), else the factory the index
         * was built with.
         */
        private IVTreeDistanceFunctionFactory resolveDistanceFunctionFactory() {
            IVTreeDistanceFunctionFactory factory =
                    (IVTreeDistanceFunctionFactory) iap.getParameters().get(IVTreeDistanceFunctionFactory.IAP_KEY);
            return factory != null ? factory : tree.distanceFunctionFactory;
        }
         ...
         * by VectorSearchOperatorNodePushable builds a quantizer from the float[6] quantization
        ...
        private void resolveAndSetQuantizer(VTreeCursorInitialState initialState, double[] queryVector,
                String distanceMetric) throws HyracksDataException {
