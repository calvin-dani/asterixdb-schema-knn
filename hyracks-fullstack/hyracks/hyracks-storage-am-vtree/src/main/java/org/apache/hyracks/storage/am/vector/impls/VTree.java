...
public class VTree extends AbstractTreeIndex {
    ...
    private void deleteVector(ITupleReference tuple, VTreeOpContext ctx) throws HyracksDataException {
        ...
        for (ClusterSearchResult clusterResult : findReplicaClusters(vector)) {
            try (ClusterAccessResult accessResult = prepareClusterAccess(clusterResult, ctx)) {
                ...
                if (!foundAndDeleted) {
                    // Scenario 1: Tuple not found in this cluster → Insert antimatter via insertIntoDataPages
    ...
    private void updateMetadataWithNewDataPage(long metadataPageId, int newDataPageId, double maxDistance,
        ...
        try {
            ...
            if (spaceNeeded > spaceAvailable) {
                ...
                // of the search-side merge and antimatter reconciliation.
                int insertPos = ((VTreeMetadataFrame) ctx.getMetadataFrame()).findInsertPosition(maxDistance);
                ctx.getMetadataFrame().insert(metadataTuple, insertPos);
            }
    ...
    public class VTreeAccessor implements ITreeIndexAccessor {
        ...
        public void search(IIndexCursor cursor, ISearchPredicate searchPred) throws HyracksDataException {
            ...
            // VectorSearchOperatorNodePushable#addAdditionalIndexAccessorParams), else the factory the
            ...
            if (distanceFunctionFactory == null) {
                distanceFunctionFactory = tree.distanceFunctionFactory;
            }
            // The full-scan predicate used by merge/flush carries no metric; fall back to the index's.
            if (distanceMetric == null) {
                distanceMetric = tree.distanceMetric;
            }
            IVTreeDistanceFunction distanceFunction = distanceFunctionFactory.createDistanceFunction(distanceMetric);
