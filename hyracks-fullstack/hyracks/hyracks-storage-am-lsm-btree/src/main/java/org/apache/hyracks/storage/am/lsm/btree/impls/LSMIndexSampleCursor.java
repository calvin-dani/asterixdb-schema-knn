...
public class LSMIndexSampleCursor extends EnforcedIndexCursor implements ILSMIndexCursor {
    ...
    protected void doOpen(ICursorInitialState initialState, ISearchPredicate searchPred) throws HyracksDataException {
        ...
        sampleLeafDrawBatchSize = ((AbstractLSMIndex) opCtx.getIndex()).getSampleLeafDrawBatchSize();
        columnSamplesPerPage = ((AbstractLSMIndex) opCtx.getIndex()).getColumnSamplesPerPage();
    ...
    private void computeDiskComponentSampleProportionality() throws HyracksDataException {
        ...
        for (int componentIdx = 0; componentIdx < operationalComponents.size(); componentIdx++) {
            ...
            ThetaEstimator.ComponentStats cStat;
            if (metadata.get(DiskComponentMetadata.THETA_INSERT_DELETE_SKETCH_KEY, thetaReference)
                    && thetaReference.getLength() > 0) {
                cStat = ThetaSampler.deserialize(thetaReference);
            } else {
                // Pre-upgrade component lacks theta metadata; treat as empty (budget = 0).
                cStat = new ThetaEstimator.ComponentStats(new long[0], new long[0], ThetaSampler.DEFAULT_K);
            }
