#!/usr/bin/env python3
"""
GIST-960 Euclidean test: compare ANN recall with and without quantization.

Usage:
    python gist_euclidean_test.py [OPTIONS]

Options:
    --limit <N>           Load only first N records (default: 100000)
    --num-clusters <N>    Number of clusters (default: 200)
    --train-list <N>      Training list size (default: 2500)
...
Examples:
    python gist_euclidean_test.py
    python gist_euclidean_test.py --skip-load --only quantized
"""
...
DATASET_FILENAME = "gist-960-euclidean_train.jsonl"
DATAVERSE = "GistTest"
...
def create_vector_index(num_clusters, train_list, quantization=None):
    """Create vector index with euclidean similarity."""
    ...
    print(f"[step] Creating vector index ({label}, similarity=euclidean, clusters={num_clusters})")
...
def run_ann_query(qvec, k, nprobe, epsilon):
    ...
    LET qvec = {qvec_str}
    FROM {DATASET_NAME} m
    LET dist = ann_distance(m.embedding, qvec, "euclidean", {nprobe}, {epsilon})
    SELECT m.idx, dist
    ORDER BY dist
    LIMIT {k};
    """
...
def run_exact_query(qvec, k):
    ...
    LET qvec = {qvec_str}
    FROM {DATASET_NAME} m
    LET dist = vector_distance(m.embedding, qvec, "euclidean")
    SELECT m.idx, dist
    ORDER BY dist
    LIMIT {k};
    """
...
def compute_recall(ann_results, exact_results):
    """Compute recall@k = |ANN intersection Exact| / k."""
...
def main():
    ...
    if args["only"] is None or args["only"] == "plain":
        ...
        result = run_test(qvec, args["k"], args["nprobe"], args["epsilon"],
                          "Plain (no quantization) + euclidean similarity")
    ...
    if args["only"] is None or args["only"] == "quantized":
        ...
        result = run_test(qvec, args["k"], args["nprobe"], args["epsilon"],
                          "Quantized (SQ7) + euclidean similarity")
        results_summary.append(result)
